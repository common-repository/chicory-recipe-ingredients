...
define('CHICORY_PLUGIN_VERSION', 		'3.1.6');
...
// Message on new installs
function install_hook($plugin) {
    if ($plugin == plugin_basename(__FILE__)) {
        set_transient('chicory_recipe_activated', true, 30);
    }
}

function installation_message() {
    if (get_transient('chicory_recipe_activated')){
        echo '<div class="notice notice-success is-dismissible"><p><strong>Chicory Shoppable Recipes</strong></p><p>Chicory makes all of your recipes shoppable at 70+ retailers and unlocks monetization opportunities with highly relevant, contextual advertising.</p></div>';
        delete_transient('chicory_recipe_activated');
    }
}

// Message on new installs
add_action('activated_plugin', 'install_hook');
add_action('admin_notices', 'installation_message');
